...
% store actual system parameters (i.e, parameter uncertainty)
Lm = L*0.8;     % CG off set along pole
th_m = pi/10;    % CG off set along pole
...
% init functions to linearize about x_bar and get jacobians
[Df_actual, Dg_actual] = jacobians(params_actual);   % <--- w/ param uncertainty
...
dt = 0.1;      % step length seconds
N = 15;         % number of steps in horizon; Horiz_time = N * dt
...
yalmip('clear')                       % good practice to clear opt vars

% super wide vectors as well
states = sdpvar(state_dim,N);      % states, [x1, ..., xN] a (4 X N) matrx
inputs = sdpvar(input_dim,N-1);    % inputs, [u1, ..., uN-1] a (1 X N-1)
...
Ad_ = sdpvar(state_dim, state_size);    % discrete drift
...
th_dot_max =  50;       % pole velocity conastraints
th_dot_min = -50;
th_f_dot_max =  200;    % flywheel velocity constraints
th_f_dot_min = -200;
...
state_weights = [1, 0, .1, .1];
input_weights = .01;

% build Qx and Qu matrices for the horizon in giant Q matrix
state_cost = kron(eye(N),diag(state_weights));
input_cost = kron(eye(N-1),diag(input_weights));
...
P = optimizer(constraints, ...                 % constraints
              objective, ...                   % objective
              options,...                      % options
              {Ad_, Bd_, Cd_, x0_},...         % input paramters
              {states,inputs});                % dec. vars.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% use LQR to compute fixed control policy, u = -K_lqr * x
% linearize about the desired state xN, x_dot = A_xN x + B_xN u
...
size_des = 500;     % has to be less than total data size
n = size(T)/size_des;
n = round(n);
T_ = T(1:n:end);
X_ = X(1:n:end,:);
...
function [Df, Dg] = jacobians(params)
    ...
    % compute jacobians
    D_th_f = zeros(4,1);
    Df = [diff(f_x,th), D_th_f, diff(f_x,th_dot), diff(f_x,th_f_dot)]; % 4X4
    Dg = [diff(g_x,th), D_th_f, diff(g_x,th_dot), diff(g_x,th_f_dot)]; % 4X4
